...
const LiveAuntMinnieQuestionFeed: React.FC<LiveAuntMinnieQuestionFeedProps> = ({
  ...
  const [collapsedPromptIds, setCollapsedPromptIds] = useState<Record<string, boolean>>({});
  ...
  const releasedPromptCount = Math.min(
    roomState.prompts.length,
    Math.max(roomState.session.current_prompt_index + 1, roomState.session.status === 'live' ? 1 : roomState.prompts.length),
  );
  const visiblePrompts = roomState.isHost
    ? roomState.prompts
    : roomState.prompts.slice(0, releasedPromptCount);
    ...
    <>
      <div className="space-y-4">
        {visiblePrompts.map((prompt, index) => {
          ...
          const responses = roomState.isHost
            ? allResponses
            : myResponse
              ? [myResponse]
              : [];
          const isCollapsed = roomState.isHost && Boolean(collapsedPromptIds[prompt.id]);
          const isQueuedForParticipants = roomState.isHost && roomState.session.status === 'live' && index >= releasedPromptCount;

          return (
            <article
              key={prompt.id}
              className={`rounded-[28px] border p-3 shadow-[0_18px_40px_rgba(3,10,18,0.18)] sm:p-4 ${
                isQueuedForParticipants
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 text-xs text-slate-400">
                    ...
                    {isQueuedForParticipants && (
                      <span className="rounded-full border border-amber-400/20 bg-amber-500/10 px-2 py-0.5 text-[11px] font-semibold text-amber-100">
                        Queued
                      </span>
                    )}
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    aria-label={roomState.isHost ? (isCollapsed ? 'Show answers' : 'Hide answers') : 'Answers'}
                    title={roomState.isHost ? (isCollapsed ? 'Show answers' : 'Hide answers') : undefined}
                  >
                    {roomState.isHost && (
